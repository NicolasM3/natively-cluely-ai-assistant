...
import {
    ...
    User, CheckCircle, ArrowUpRight, ChevronRight, Paperclip, FileText, FolderOpen,
...
// Adopt any ingest still running in main. Closing the panel unmounts this
        ...
        window.electronAPI?.profileGetFolderStatus?.().then((status: any) => {
            if (status) setFolderStatus(status);
        }).catch(() => {});
    }, []);
    ...
    const browseProfileFolder = async () => {
        setFolderError('');
        const picked = await window.electronAPI?.profileSelectFolder?.();
        if (!picked || picked.cancelled) return;
        if (!picked.folderPath) {
            setFolderError(picked.error || 'Could not select folder.');
            return;
        }
        setFolderSyncing(true);
        try {
            const result = await window.electronAPI?.profileSyncFolder?.(picked.folderPath);
            if (!result?.success) {
                setFolderError(result?.error || 'Folder sync failed.');
                return;
            }
            if (result.status) {
                setFolderStatus({
            ...
            const refreshedFolder = await window.electronAPI?.profileGetFolderStatus?.();
            if (refreshedFolder) setFolderStatus(refreshedFolder);
            const [status, data] = await Promise.all([
                window.electronAPI?.profileGetStatus?.(),
                window.electronAPI?.profileGetProfile?.(),
            ]);
            ...
            setFolderError(e?.message || 'Folder sync failed.');
        } finally {
            setFolderSyncing(false);
        }
    };

    const resyncProfileFolder = async () => {
        if (!folderStatus?.folderPath) return;
        setFolderError('');
        setFolderSyncing(true);
        try {
            const result = await window.electronAPI?.profileSyncFolder?.(folderStatus.folderPath);
            if (!result?.success) {
                setFolderError(result?.error || 'Folder sync failed.');
                return;
            }
    ...
    const renderIdentity = () => {
        const hasProfileLoaded = profileStatus.hasProfile || Boolean(folderStatus?.hasProfile);
        const isActive = profileStatus.profileMode && hasProfileAccess;
        const isDisabled = !hasProfileLoaded || !hasProfileAccess;
        const personaSubtitle = profileStatus.profileMode
            ? 'Answers rewired around your profile, the role, and your voice.'
            : !hasProfileLoaded
                ? 'Load a resume or sync a profile folder to enable.'
                : 'Turn on Persona Engine to shape answers with your profile.';
        return (
        <>
            ...
            >
                <div>
                    ...
                    {personaError && (
                        <p style={{ fontSize: 12, color: 'var(--pi-danger)', margin: '6px 0 0' }}>{personaError}</p>
                    )}
                </div>
                <button
                    type="button"
                    role="switch"
                    data-on={String(!!(profileStatus.profileMode && hasProfileAccess && hasProfileLoaded))}
                    aria-checked={!!(profileStatus.profileMode && hasProfileAccess && hasProfileLoaded)}
                    aria-disabled={isDisabled ? true : undefined}
                    aria-label="Persona Engine"
                    onClick={async () => {
                        if (!hasProfileLoaded || !hasProfileAccess) return;
                        const newState = !profileStatus.profileMode;
                        setPersonaError('');
                        try {
                            const result = await window.electronAPI?.profileSetMode?.(newState);
                            if (result && !result.success) {
                                setPersonaError(result.error || 'Could not update Persona Engine.');
                                return;
                            }
                            piToggleInit.arm();
                            setProfileStatus(prev => ({ ...prev, profileMode: newState }));
                        } catch (e: any) {
                            setPersonaError(e?.message || 'Could not update Persona Engine.');
                        }
                    }}
            ...
            <div style={{ marginBottom: 20, padding: '12px 14px', border: '1px solid var(--pi-border)', borderRadius: 'var(--pi-r-md)', background: 'rgba(255,255,255,0.02)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
                    <div>
                        <h3 className="pi-section-label" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                            <FolderOpen size={14} /> Profile Folder
                        </h3>
                        <p style={{ fontSize: 12, color: 'var(--pi-secondary)', margin: '4px 0 0', lineHeight: 1.5 }}>
                            Point Natively at a folder with your CV, notes, and interview prep. Supported: PDF, HTML, MD, CSV, TXT.
                        </p>
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                        <button
                            ...
                            onClick={browseProfileFolder}
                            style={{ fontSize: 12, padding: '6px 10px', borderRadius: 8, border: '1px solid var(--pi-btn-border)', background: 'var(--pi-btn-bg)', color: 'var(--pi-primary)', cursor: folderSyncing ? 'not-allowed' : 'pointer', opacity: folderSyncing ? 0.6 : 1 }}
                        >
                            {folderStatus?.folderPath ? 'Change' : 'Select folder'}
                        </button>
                        {folderStatus?.folderPath && (
                            <button
                                ...
                                className="pi-press-soft"
                                disabled={folderSyncing}
                                onClick={resyncProfileFolder}
                                title="Re-read all files from the folder"
                                style={{ fontSize: 12, padding: '6px 8px', borderRadius: 8, border: '1px solid var(--pi-btn-border)', background: 'var(--pi-btn-bg)', color: 'var(--pi-secondary)', cursor: folderSyncing ? 'not-allowed' : 'pointer', opacity: folderSyncing ? 0.6 : 1, display: 'flex', alignItems: 'center' }}
                            >
                                <RefreshCw size={13} />
                            </button>
                        )}
                ...
                {folderSyncing && (
                    <p style={{ fontSize: 12, color: 'var(--pi-secondary)', margin: '0 0 8px' }}>Reading profile folder…</p>
                )}
                {folderError && (
                    <p style={{ fontSize: 12, color: 'var(--pi-danger)', margin: '0 0 8px' }}>{folderError}</p>
                )}
                {folderStatus?.folderPath ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        <div style={{ fontSize: 11, color: 'var(--pi-tertiary)', wordBreak: 'break-all' }}>
                            {folderStatus.folderPath}
                        </div>
                        <div style={{ fontSize: 11, color: 'var(--pi-secondary)' }}>
                            {folderStatus.fileCount} file{folderStatus.fileCount === 1 ? '' : 's'}
                            {folderStatus.profileName ? ` · ${folderStatus.profileName}` : ''}
                            {folderStatus.lastSyncedAt ? ` · synced ${new Date(folderStatus.lastSyncedAt).toLocaleString()}` : ''}
                            {folderStatus.vectorIndex?.indexedChunks
                                ? ` · ${folderStatus.vectorIndex.indexedChunks} vector chunk${folderStatus.vectorIndex.indexedChunks === 1 ? '' : 's'}`
                                : ''}
                            {folderStatus.vectorIndex?.indexingInFlight
                                ? ' · indexing…'
                                : folderStatus.vectorIndex?.indexedChunks
                                    ? folderStatus.vectorIndex.embeddingsReady
                                        ? ' · embeddings ready'
                                        : ' · embeddings pending'
                                    : ''}
                        </div>
                        {folderStatus.files.length > 0 && (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                                {folderStatus.files.slice(0, 6).map((file) => (
                                    <span key={file} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 'var(--pi-r-pill)', background: 'var(--pi-btn-bg)', border: '1px solid var(--pi-btn-border)', color: 'var(--pi-secondary)' }}>
                                        {file}
                                    </span>
                                ))}
                            </div>
                    ...
                    <p style={{ fontSize: 12, color: 'var(--pi-tertiary)', margin: 0 }}>
                        Example: <code style={{ fontSize: 11 }}>~/Documents/personal/nicolas-oliveira</code>
                    </p>
                )}
            </div>
